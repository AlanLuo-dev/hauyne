...
public class LogicDeleteByBatchIds extends AbstractMethod {
    ...
    public LogicDeleteByBatchIds(String methodName, LogicDeleteProperties logicDeleteProperties) {
        super(methodName);
    ...
    public MappedStatement injectMappedStatement(Class<?> mapperClass, Class<?> modelClass, TableInfo tableInfo) {
        ...
        final String forEachSql = SqlScriptUtils.convertForeach("#{item}", "ids", null, "item", COMMA);
        final String whereIdIn = " WHERE " + keyColumn + " IN (" + forEachSql + ");";

        String sql = "<script>\n" +
                " INSERT INTO " + deletedTableName + "( " + deletedTableFields + " )" +
                " SELECT " + fields + ", #{deletedBy}, #{deletedTime} FROM " + tableName + whereIdIn +
                " DELETE FROM " + tableName + whereIdIn +
                "\n</script>";
