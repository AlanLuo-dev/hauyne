 ...
 */
public class EnumToObjectSerializer<T extends Serializable> extends JsonSerializer<CodeEnum<T>> {

    @Override
    public void serialize(CodeEnum<T> enumValue, JsonGenerator gen, SerializerProvider serializers) throws IOException {
        // 开始序列化对象（生成 { ）
    ...
    public Class<CodeEnum<T>> handledType() {
        return (Class<CodeEnum<T>>) (Class<?>) CodeEnum.class;
    }
